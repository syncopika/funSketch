...
function Filters(canvas, brush){
	...
	***/
	...
	/***
		'HEATWAVE' FILTER
		this filter saturates and darkens some colors and produces an interesting pavarte
	***/
	...
	this.areaColor = function(pixels){
		...
		for(var i = 0; i < d.length; i+=4){
			...
			if(!cond1 && !cond2 && !cond3 && !cond4 && !cond5 && !cond6 && !cond7 && !cond8){		
				//if next neighbor over is a compvarely different color, stop and move on
				var nnr = copy[i+8];
				var nng = copy[i+9];
				var nnb = copy[i+10];		
	...
	***/
	this.mosaic = function(pixels){
		...
		// make sure chunkWidth can compvarely divide the image width * 4 
	...
	/***
		...
		because my traversal of the chunks in the array is compvarely wrong 
