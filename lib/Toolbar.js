...
function Toolbar(canvas, brush, animationProj) {
    ...
    this.up = function(){
        ...
        if(canvas.currentIndex + 1 < canvas.canvasList.length) {
			...
			canvas.currentCanvas.style.cursor = "";
            ...
            if(canvas.currentIndex > 0) {
				let prevLayer = canvas.canvasList[canvas.currentIndex - 1];
			...
			let nextLayer = canvas.canvasList[canvas.currentIndex + 1];
            nextLayer.style.opacity = .97;
            nextLayer.style.zIndex = 1;
			nextLayer.style.cursor = "crosshair";
			
            canvas.currentCanvas = nextLayer;
            canvas.currentIndex++;
            
			// apply brush on new current canvas
			// TODO: keep using currently selected brush!
            brush.defaultBrush();
            return true;
        }
        return false;
    ...
    this.down = function(){
        ...
        if(canvas.currentIndex - 1 >= 0){
			...
			let prevLayer = canvas.canvasList[canvas.currentIndex - 1];
            prevLayer.style.opacity = .97;
            prevLayer.style.zIndex = 1;
			prevLayer.style.cursor = "crosshair";
            ...
            canvas.currentCanvas = prevLayer;
            canvas.currentIndex--;
    ...
    this.nextFrame = function(){
        ...
        if(next !== null){
            curr.hide();
            next.show();
            brush.defaultBrush(); // brush shouldn't change?
            return true;
        }
        return false;
    };
