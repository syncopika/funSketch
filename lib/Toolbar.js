...
function Toolbar(canvas, brush, animationProj) {
    ...
    this.up = function(){
        ...
        if(canvas.currentIndex + 1 < canvas.canvasList.length) {
			...
			canvas.canvasList[canvas.currentIndex + 1].style.cursor = "crosshair";
	...
	***/
	this.deleteCurrentFrameButton = function(elementId, optionalFunction){
        $('#' + elementId).click(() => {
			...
			if(animationProj.deleteFrame(currFrameIdx)){	
				optionalFunction(currFrameIdx); // the index of the frame we deleted
			}
        });
	}
